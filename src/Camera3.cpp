...
public:
    ...
    vetor phong_shading(ray& r, vector<sphere*>& objetos, const vector<light>& lights, vetor ambient_light, int index)
    {
        PerlinNoise perlin;  // Create a PerlinNoise instance
        ...
        if (index <= 10) {   // pode ser mudado
            ...
            if (t != INFINITY) {
                ...
                vetor objeto_color = getCheckerboardColorSphere(objetos[ind], intersection);
    ...
    void render(vector<sphere*>& objetos, const vector<light>& lights, const vetor& ambient_light) {
